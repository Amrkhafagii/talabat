...
export default function EditMenuItem() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const item = await getMenuItemById(String(itemId));
      if (!item) {
        Alert.alert('Error', 'Menu item not found', [{ text: 'OK', onPress: () => router.back() }]);
        return;
      }
      const cats = await getCategories(item.restaurant_id);
      setMenuItem(item);
      setCategories(cats);
      setName(item.name);
  ...
  const handleAddCategory = async () => {
    if (!newCategory.trim() || !menuItem) return;
  ...
  const handleSave = async () => {
    if (!menuItem || !validate()) return;
    try {
      setSaving(true);
      let photoUrl = imageUri.trim();
      if (photoUrl && !photoUrl.startsWith('http')) {
        const uploaded = await uploadImage(photoUrl, menuItem.restaurant_id);
        if (!uploaded) {
      ...
      const payload: Partial<MenuItem> = {
        ...
        category: categories.find((c) => c.id === categoryId)?.name ?? menuItem.category,
        is_available: isAvailable,
        is_popular: isPopular,
        photo_approval_status: imageChanged ? 'pending' : undefined,
      };

      const { success, error, errorCode } = await updateMenuItem(menuItem.id, payload, menuItem.restaurant_id);
      if (success) {
        Alert.alert('Saved', 'Menu item updated', [{ text: 'OK', onPress: () => router.back() }]);
      } else {
        const friendly = errorCode === '42501' ? 'Not allowed to edit this item' : 'Failed to update item';
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <ScreenHeader title="Edit Menu Item" onBack={() => router.back()} />
  ...
  if (!menuItem) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <ScreenHeader title="Edit Menu Item" onBack={() => router.back()} />
        <View style={styles.loadingContainer}>
          <Text style={styles.loadingText}>Menu item not found.</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: theme.insets.bottom + theme.spacing.xl }} showsVerticalScrollIndicator={false}>
        ...
        <Button title={saving ? 'Saving...' : 'Save Changes'} onPress={handleSave} disabled={saving} style={styles.saveButton} pill fullWidth />
      </ScrollView>
    </SafeAreaView>
  );
}
