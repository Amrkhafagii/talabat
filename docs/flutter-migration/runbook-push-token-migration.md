...
- Supabase tables: `push_tokens` (columns: `user_id`, `token`, `platform`, `updated_at`), `telemetry_events` (filter `event_name = 'push_registration'`), `telemetry_crashes`.
- Scripts: `flutter/packages/app_services/lib/src/services/push_registration_service.dart`.

## Monitoring
1. **Token intake** – Grafana panel “FCM token registrations” (querying Supabase `push_tokens` grouped by `platform`). Alert if Flutter registrations <80% of active DAU for 12h.
...
3. Audit that no Expo tokens remain (Phase 9 migration renamed `user_push_tokens` to `push_tokens`):
   ```sql
   SELECT count(*) AS expo_rows
   FROM push_tokens
   WHERE platform ILIKE 'expo%';
   ```
   The query must return `0`. Escalate to ops if any Expo-originated row shows up; they should be deleted immediately.
